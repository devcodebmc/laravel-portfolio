<section id="projects" class="relative py-24 px-6 lg:px-16 overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto relative">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div class="space-y-3">
                ...
                <p class="text-[#5a5a7a] text-base font-light max-w-xl leading-relaxed">
                    A selection of projects I've built for clients and personal use.
                </p>
            </div>
            
            <div class="flex items-center gap-3 shrink-0">
                <span class="text-xs text-[#8a8aaa] font-mono tracking-wider">FILTER BY</span>
                <span class="w-px h-6 bg-[#e0e0e6]"></span>
                <div class="flex gap-1.5 bg-white/80 backdrop-blur-sm p-1.5 rounded-full shadow-sm border border-[#e8e8f0]">
                    <button data-filter="*" class="filter-btn px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-300 bg-[#807fe2] text-white shadow-lg shadow-[#807fe2]/20">
                        All
                    </button>
                    <button data-filter=".laravel" class="filter-btn px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-300 text-[#5a5a7a] hover:text-[#1a1a2e] hover:bg-[#f0edff]">
                        Laravel
                    </button>
                    <button data-filter=".wordpress" class="filter-btn px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-300 text-[#5a5a7a] hover:text-[#1a1a2e] hover:bg-[#f0edff]">
                        WP
                    </button>
                    <button data-filter=".php" class="filter-btn px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-300 text-[#5a5a7a] hover:text-[#1a1a2e] hover:bg-[#f0edff]">
                        PHP
                    </button>
                    <button data-filter=".landing" class="filter-btn px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-300 text-[#5a5a7a] hover:text-[#1a1a2e] hover:bg-[#f0edff]">
                        Landing
                    </button>
                </div>
            </div>
        ...
        <div id="project-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-6">
            @php
                $projects = [
                    [
                    ...
                    <div class="relative rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1 border border-[#f0edff] hover:border-[#807fe2]/20 h-full flex flex-col">
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <div class="flex items-start justify-between gap-2">
                                <h3 class="text-sm font-semibold text-[#1a1a2e] group-hover:text-[#807fe2] transition-colors truncate flex-1">
                                    {{ $project['title'] }}
                                </h3>
                                <div class="flex items-center gap-1.5 shrink-0">
                                    @if($project['github'])
                                        <a href="{{ $project['github'] }}" target="_blank" rel="noopener noreferrer"
                                           class="w-7 h-7 rounded-full bg-[#f0edff] flex items-center justify-center border border-[#807fe2] text-[#807fe2] hover:bg-[#807fe2] hover:text-white transition-all duration-300 hover:scale-110">
                            ...
                            <p class="text-[12px] text-[#8a8aaa] font-mono truncate mt-0.5">
                                {{ $project['category'] }}
                            </p>

                            <div class="flex flex-wrap gap-1 mt-2">
                                @foreach($project['tags'] as $tag)
                                    <span class="text-[11px] font-medium px-2 py-0.5 rounded-full bg-[#f0edff] text-[#807fe2]">
                                        #{{ $tag }}
                                    </span>
                                @endforeach
                            </div>
        ...
        <div class="mt-16 text-center">
            <div class="inline-flex flex-col sm:flex-row items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-[#e8e5f5] shadow-lg">
                ...
                <a href="#contact" 
                   class="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-gradient-to-r from-[#807fe2] to-[#6c63d9] text-white font-medium text-sm shadow-lg shadow-[#807fe2]/30 hover:shadow-[#807fe2]/50 hover:-translate-y-1 transition-all duration-300 group">
