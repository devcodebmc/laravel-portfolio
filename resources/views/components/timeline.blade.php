    ...
    <div class="max-w-7xl mx-auto relative">
        <div class="text-center mb-16">
            <div class="flex items-center justify-center gap-4 mb-4">
                ...
                <span class="text-[11px] font-mono text-[var(--text-faint)] tracking-wider">// journey</span>
            </div>
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold leading-[1.08]">
                <span class="text-[var(--text-primary)]">Where I've</span>
                    ...
                    @foreach($experiences as $index => $exp)
                        <div class="relative mb-8 last:mb-0 group">
                            <div class="absolute -left-[2.15rem] md:-left-[2.75rem] top-1.5">
                                <div class="relative">
                                    <div class="w-4 h-4 rounded-full border-2 border-[#807fe2] bg-[var(--bg-card)] group-hover:bg-[#807fe2] transition-all duration-300 shadow-sm group-hover:shadow-md group-hover:shadow-[#807fe2]/30 group-hover:scale-110"></div>
                                    @if($exp['featured'])
                            ...
                            <div class="bg-[var(--bg-card)] rounded-xl p-6 md:p-7 shadow-sm hover:shadow-xl transition-all duration-400 hover:-translate-y-1 border border-[var(--border-color)] hover:border-[#807fe2]/25 group-hover:shadow-[#807fe2]/5">
                                {{-- Header --}}
                                <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3.5">
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2.5 flex-wrap">
                                            <h3 class="text-lg md:text-xl font-bold text-[var(--text-primary)] group-hover:text-[#807fe2] transition-colors">
                                                {{ $exp['company'] }}
                                            </h3>
                                            @if($exp['featured'])
                                    ...
                                    <span class="text-xs font-mono text-[var(--text-faint)] whitespace-nowrap shrink-0 bg-[var(--bg-section-alt)] px-3 py-1 rounded-full border border-[var(--border-color)]">
                                        {{ $exp['period'] }}
                                    </span>
                                </div>

                                <p class="text-sm text-[var(--text-muted)] font-light leading-relaxed mb-3.5">
                                    {{ $exp['description'] }}
                                </p>
                            </div>
                        </div>
